/** 임의공급 크롤러. proxy/crawlers/opt.py 포팅. */

import { OPT_API_URL, OPT_MDL_API_URL } from "../_shared/config.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [allItems, sizeMap] = await Promise.all([
      fetchAllPages(OPT_API_URL, startmonth, endmonth),
      fetchSizeMap(OPT_MDL_API_URL, startmonth, endmonth),
    ]);
    ...
    for (const item of allItems) {
      ...
      const ann = normalizeApplyhome(item, "opt", "임의공급");
      if (!ann || seenIds.has(ann.id)) continue;
      const rawId = ann.id.replace("opt_", "");
      ann.size = sizeMap[rawId] ?? "";
      seenIds.add(ann.id);
      results.push(ann);
    }

    const { inserted, errors } = await upsertAnnouncements(results, "opt", monthsBack);

    console.log(`임의공급: ${inserted} announcements (${startmonth}~${endmonth})`);
    return jsonResponse({ category: "opt", inserted, total_fetched: results.length, errors: errors.length ? errors : null });
  } catch (e) {
    console.error(`crawl-opt error: ${e}`);
