...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    ...
    return jsonResponse({ user_id: userId, profile: data.profile, score, upcoming_alert: upcomingAlert, updated_at: data.updated_at, recalculated: stale });
  }
  ...
  if (req.method === "POST") {
    ...
    const profile: UserProfile = {
      ...
      parents_registered_since: body.parents_registered_since ? String(body.parents_registered_since) : undefined,
    };
    ...
    return jsonResponse({ user_id: userId, score, upcoming_alert: upcomingAlert, saved: true });
  }
