...
| **C** | 프로필 CRUD (`GET/PATCH /v1/profile`) | B + main의 user_profiles | 4~6h | ✅ **stacked PR** (`feat/profile-recommendations`) — user_profiles JSONB 패턴, my-score와 호환 |
| **D** | 추천 (`GET /v1/recommendations`) | C | 3~4h | ✅ **stacked PR** — user_profiles.profile JSONB에서 추출 |
...
**별도 PR 작업** (`feat/profile-recommendations` — stacked on PR #1):
- ✅ `_shared/profile.ts` — UI 필드(`preferred_regions`, `special_supply_interests` 등) ↔ UserProfile(`birth_date`, `savings_start` 등) 통합 타입 + derived fields(age, homeless_years 등) helper
- ✅ `supabase/functions/profile/index.ts` — `user_profiles.profile JSONB` 부분 머지 패턴, my-score와 호환 (score JSONB는 my-score가 관리)
- ✅ `supabase/functions/recommendations/index.ts` — `user_profiles.profile` JSONB에서 매칭 필드 추출 (preferred_regions 우선, resident_region 폴백)

**스키마 마이그레이션 불필요** — `user_profiles` 테이블은 운영 DB에 이미 적용됨. 본 후속 PR은 코드만 추가/변경.

**API 변경점** (정규화 컬럼 → JSONB):
- GET `/v1/profile` 응답: `{ user_id, profile: {...JSONB...}, derived: {age, homeless_years, ...}, score, updated_at }`
- PATCH `/v1/profile` body: 화이트리스트 18개 필드 (UserProfile 12 + UI extras 6)

본 PR(`feat/notice-and-foundation`)에서는 `_shared/auth.ts` + notifications + reports만 보내고, profile + recommendations는 후속 PR(`feat/profile-recommendations`)로 분리.
