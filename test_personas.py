"""8명 페르소나 E2E 시뮬레이션 테스트 — 가점 계산은 Edge Function 직접 호출."""
import os
import sys
from datetime import date
import requests

PROXY_URL = os.environ.get("PROXY_URL", "http://localhost:8002")
...
def persona_to_userprofile(prof):
    """test persona schema → Edge Function UserProfile (eligibility-precheck body)."""
    today = date.today()
    birth_date = f"{prof['birth_year']}-07-01"

    has_acct = prof["subscription_account"]["has_account"]
    savings_years = prof["subscription_account"]["years"]
    if has_acct and savings_years > 0:
        savings_start = f"{today.year - savings_years}-{today.month:02d}-01"
    else:
        savings_start = today.isoformat()

    is_married = prof["household"]["type"] in ("newlywed", "married_no_child", "married_with_child")
    marriage_date = None
    if prof.get("marriage_date"):
        parts = prof["marriage_date"].split("-")
        marriage_date = f"{parts[0]}-{parts[1]}-01"
    ...
    return {
        ...
        "is_homeless": prof["homeless"],
        "savings_start": savings_start,
        "savings_balance_wan": prof["subscription_account"]["deposit_count"] * 2,
        "resident_region": prof.get("residence_region", "서울"),
        "has_house": prof["housing_count"] > 0,
        "parents_registered": False,
    }


def fetch_score(user_profile, announcement_id):
    """eligibility-precheck Edge Function 호출 — 인증 불필요, score 반환."""
    payload = {"announcement_id": announcement_id, **user_profile}
    try:
        resp = requests.post(
            f"{SUPABASE_URL}/functions/v1/eligibility-precheck",
            headers={
        ...
        return resp.json().get("score"), None
...
def main():
    try:
        requests.get(f"{PROXY_URL}/health", timeout=15)
    except Exception as e:
    ...
    for attempt in range(3):
        try:
            resp = requests.get(f"{PROXY_URL}/announcements", params={
                "category": "all", "active_only": "false", "months_back": "2"
            }, timeout=120)
            resp.raise_for_status()
            anns = resp.json()["announcements"]
            break
        except Exception as e:
    ...
    ref_id = anns[0]["id"] if anns else None
    edge_ok = bool(SUPABASE_URL and SUPABASE_ANON_KEY and ref_id)
    if not edge_ok:
        print("[warn] SUPABASE_URL / SUPABASE_ANON_KEY 미설정 — 가점 E2E 스킵")
    ...
    for p in personas:
        ...
        if edge_ok:
            score, err = fetch_score(persona_to_userprofile(prof), ref_id)
            if err:
                print(f"  가점: 조회 실패 ({err})")
            elif score:
                print(
                    f"  추정 가점: {score['total']}점/84점"
                    f" (무주택 {score['homeless_score']} +"
                    f" 부양가족 {score['dependents_score']} +"
                    f" 통장 {score['savings_score']})"
                )
        else:
            print("  가점: 스킵")
...
def run_mock_tests():
    """단위 테스트 — filter_anns + D-day 정렬 (가점 계산은 Edge Function으로 이관)."""
    print("\n[Mock Tests]")
    mock_anns = [
    ...
    strict_profile = {"regions": ["서울"], "preferred_size": ["중형"]}
