...
청약홈(applyhome.co.kr) + LH(apply.lh.or.kr) 공고 상세 페이지의 본문 텍스트를
정규화하여 반환한다. LLM이 사용자 프로필 컨텍스트로 요약·해석하기 위한 입력.
...
def _extract_lh(html: str) -> dict:
    ...
    LH는 게시판 구조라 .board-view / .view-content / #content 등이 흔하다.
    셀렉터 실패 시 body 전체 fallback.
    """
    soup = BeautifulSoup(html, "html.parser")
    title = _extract_title(soup)
...
def extract_notice_raw(
    ...
    # 캐시는 항상 풀텍스트로 보관, 응답 시 max_chars로 잘라낸다 (부하 절감)
