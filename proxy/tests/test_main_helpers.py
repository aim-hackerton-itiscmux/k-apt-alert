...
def test_ttl_for_no_colon():
    """':'가 없어도 split이 동작."""
    # 'apt'만 있으면 prefix='apt' → CACHE_TTLS["apt"]
...
def test_build_messages_handle_missing_fields():
    ...
    bad = {"d_day": 5}  # 거의 모든 필드 없음
    main._build_slack_blocks([bad])  # 예외 없어야
