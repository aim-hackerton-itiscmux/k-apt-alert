...
def get_notice_raw(
    ...
    - 호스트는 화이트리스트 검증 (applyhome.co.kr / apply.lh.or.kr)
    - 무료 30K, 유료 50~80K (Phase 1은 무료 강제)
    - 7일 캐시, force_refresh=true로 무효화
    """
    # 티어 해석 + max_chars 클램프
    effective_tier = _resolve_tier(authorization)
    if tier == "paid" and effective_tier != "paid":
        # 인증 실패 시 free로 강등 (에러 X)
        pass
    ...
    if not is_supported_host(resolved_url):
        raise HTTPException(
            status_code=400,
            detail=(
                f"unsupported host. Phase 1 supports applyhome.co.kr and apply.lh.or.kr only. "
                f"Got: {resolved_url}"
            ),
        )
